...
namespace aasdk {
  namespace messenger {
    ...
    // Windows PoC patch: let TLS report plaintext length; record overhead is cipher-dependent.
    size_t Cryptor::decrypt(common::Data &output, const common::DataConstBuffer &buffer, int frameLength) {
      if (frameLength <= 0 || static_cast<size_t>(frameLength) != buffer.size)
        throw error::Error(error::ErrorCode::SSL_READ, 0, "Invalid encrypted frame size");
      std::lock_guard<decltype(mutex_)> lock(mutex_);
      this->write(buffer);
      const auto initialSize = output.size();
      unsigned char plaintext[16384];
      for (;;) {
        const auto count = sslWrapper_->sslRead(ssl_, plaintext, sizeof(plaintext));
        if (count > 0) {
          output.insert(output.end(), plaintext, plaintext + count);
          continue;
        }
        const auto nativeError = sslWrapper_->getError(ssl_, count);
        if (nativeError == SSL_ERROR_WANT_READ) break;
        throw error::Error(error::ErrorCode::SSL_READ, nativeError, "TLS record decode");
      }
