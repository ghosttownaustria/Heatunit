  ...
  namespace channel {
    namespace control {
      ...
      void ControlServiceChannel::handleVersionResponse(const common::DataConstBuffer &payload,
        ...
        if (payload.size < 6) {
          eventHandler->onChannelError(error::Error(error::ErrorCode::PARSE_PAYLOAD, 0, "Short version response"));
          return;
        }
        const auto read16 = [&](size_t offset) { return static_cast<uint16_t>((payload.cdata[offset] << 8) | payload.cdata[offset + 1]); };
        const auto major = read16(0), minor = read16(2);
        const auto status = static_cast<aap_protobuf::shared::MessageStatus>(static_cast<int16_t>(read16(4)));
        AASDK_LOG(info) << "[ControlServiceChannel] Version " << major << "." << minor << " status=" << status;
        eventHandler->onVersionResponse(major, minor, status);
      }
