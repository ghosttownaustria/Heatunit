...
namespace aasdk {
  namespace transport {
    ...
    int SSLWrapper::getError(SSL *ssl, int returnCode) {
      ...
      // A memory BIO normally needs more peer data after draining a TLS record.
      if (sslErrorCode == SSL_ERROR_NONE || sslErrorCode == SSL_ERROR_WANT_READ || sslErrorCode == SSL_ERROR_WANT_WRITE)
        return sslErrorCode;
