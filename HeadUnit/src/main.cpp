...
{
    bool isScanOnly = false, isSmokeTest = false, isUsbProbe = false, isStartAccessory = false, isProjectionTest = false;
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--scan") isScanOnly = true;
            ...
            std::cout << "HeadUnit [--scan | --smoke-test | --probe-usb | --start-accessory | --test-projection]\nLogs: ./headunit.log (includes USB serial numbers)\n";
            return 0;
        } else { std::cerr << "Unknown option: " << argument << '\n'; return 1; }
    }
    if (static_cast<int>(isScanOnly) + static_cast<int>(isSmokeTest) + static_cast<int>(isUsbProbe) + static_cast<int>(isStartAccessory) + static_cast<int>(isProjectionTest) > 1) { std::cerr << "Choose one run mode\n"; return 1; }
    try {
